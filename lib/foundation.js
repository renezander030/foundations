// foundation.js — small, self-contained scaffold for agent-first CLIs.
// Provides JSON output by default with a -H/--human table fallback, a subcommand
// router, --key val / --key=val flag parsing, a TTL'd JSON cache, an HTTP-JSON
// helper, fuzzy id/title matching, and a forgiving date parser. Zero deps.
//
// Usage:
...
// --- .env loader (KEY=value, one per line) ---
...
// --- output: JSON by default, table when --human/-H present ---
...
// out(data) — terse JSON line.
export function out(data) { console.log(JSON.stringify(data)); }

// --- flag parsing (--key val and --key=val) ---
...
// parseFilters(args, ['type','lang']) -> filters object (--k v / --k=v).
...
// --- HTTP JSON helper (http+https aware) ---
...
// --- date parser (today/tomorrow/YYYY-MM-DD/Nd|Nw|Nm) ---
// Returns ISO with a +0000 offset; pass `iso:true` for plain ISO instead.
export function parseDate(s, { iso = false } = {}) {
  ...
  // Noon (not 23:00) so a +1/+2h local offset never crosses midnight into the next day.
